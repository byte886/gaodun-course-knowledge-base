...
### 变更
- 知识生成 SOP 新增「§六 重生成既有课程（在已有成品上重做，一门一闭环）」（2026-09-19；用户拍板其余 7 门本地知识详解按多智能体方法重做）：把重做一门课的常驻规则在流水线里固化一次——①结构对账（官方大纲 course-manifest/syllabus 为准，现算本地章/点、查清多缺，不增丢官方点）→②整目录快照到 `data/_workspace/<profile>/regen-backup/<时间戳>/`（完整才覆盖）→③同名同结构原地重写（章/点文件名、标题、层级不变，飞书树与 wiki_node_map 稳定、同步即正文覆盖），仍按 §三多智能体逐章生成→独立检查 6 维→打回≤2、章章 okf_validate E=0→④§四深度审核（15–20 篇）+§五全量自检→⑤人工闸口停下交《闸口报告》等用户"通过/打回+章节"→⑥通过后才按 wiki-sync-sop 建树/resync/双验收（新空间先归档老 map）、闭环后再开下一门；做题账号风控冻结不新做题、题答只用已采集 papers、无题点标 N/A。做哪几门/顺序/进度属任务状态不固化。`knowledge-sop-overview.md` ③导航补指 §六；TASK_STATUS 交接块据此瘦身为"路线状态+环境红线"，规则不双写。目标＝交接提示词与任务台账最小化，"怎么做"只由 SOP 承载。
- 飞书知识库同步链路重构为**新空间 8 课模型**并固化为配置驱动脚本/SOP（2026-09-19，会计已闭环样板）：新空间「CPA备考知识库」`7686897234545249236`、根「课程库」`K4IcwWg6HidyY6k6Z8hcdmGFnth`，层级＝课程库→全称课程容器（容器本身=课程首页）→章/全局篇→知识点。①`scripts/knowledge/build_tree.py` 重写：读配置卡 `primaryCourse.name`+`wiki` 块幂等建全称容器、回写 `courseNodeToken/courseObjToken`、在内容器建章→点→全局篇（兼容旧 3 参）；②`resync_wiki_content.py` profile 懒加载容错、增 `--profile/--course-obj/--no-homepage`，**根 README 写入容器=课程首页**（done `__COURSE_HOMEPAGE__.done`）、frontmatter 自动剥离；③`run_resync_batches.sh`/`sync_with_restart.sh` 去硬编码 REPO、TOTAL 自动=map+首页、显式 `--profile`；④新增只读通用双验收 `verify_wiki_tree.py`（本地/map/飞书容器树三方动态计数比对）与 `verify_wiki_content.py`（逐篇 docs+fetch 判 OK/THIN/EMPTY/FETCH_FAIL、`--refill` 补非 OK）；⑤8 张配置卡全部合法化并挂新空间 wiki 块（会计补齐 key/subject/saasCourseId 过 course_profile 校验并回写容器 token）；⑥重写 `docs/development/guides/wiki-sync-sop.md` 为 8 课模型，同步 scripts/README §十、WORKFLOW 飞书同步策略、wiki-link-verification-sop、feishu-api 引用；⑦退役 `sync_wiki_new.sh`、`auto_sync_all.py/.sh`、`verify_sync_completeness.py` 入 `.trash/*.20260919`。会计课实测：169 子页+课程首页共 170 篇正文回读 0 空/0 薄/0 失败、结构验收 0 差异；并修复本地第 17 章目录名被截断 `17_收入`→`17_收入、费用和利润`（改目录名+跨章相对链接+map 标题键，token 不变）。明确限流真因＝豆包转发代理按单进程累计请求（换新进程即恢复、外层约 3 轮换新 shell、单进程 20 篇 0 失败），证伪旧"简单逐个脚本最可靠"结论。OKF concept workflow-feishu-sync 增补本模型。其余 7 课待按新 SOP 逐门建树+同步+双验收。
...
- 税法课知识系统 finalize 收尾与决策沉淀（2026-09-07）：新增 **ADR-013**（知识库元数据边界：最终产物只保留纯知识内容，学员名/点赞数/用户ID/日期等元数据一律不进知识库；原始资料保留完整字段用于溯源筛选）、**ADR-014**（考试成绩与 AI 批改边界：每套考试有 11–15 分 AI 不支持批改的主观题，不可控，不追求刷满分；知识详解按知识点全覆盖优先级高于考试分数；做题首要目的=获取知识来源而非刷分）、`docs/development/guides/wiki-link-verification-sop.md`（飞书知识库链接验证 SOP：三层验证法——总览页 28 链接→章 README 92 链接→知识点详解 654 链接，回读提取 node_token 与 wiki_node_map.tsv 比对，含常见问题修复与验收标准）
- 工程记忆层 OKF bundle 落地（2026-09-07，决策见 ADR-017，用户硬约束「工程自包含、可移植到其它 Agent、不依赖全局 AGENTS/技能」）：新增 `docs/project-management/memory/` 工程记忆包——`index.md` 分组导航 + `log.md` 倒序变更线 + `concepts/` 10 篇高密度 concept（架构 4：四地存储分工 / 统一运行时工作区 / 三层解耦范式 / 元数据边界；链路 4：浏览器 CDP / 做题接口链路 / 视频压缩转写 / 飞书同步单窗口；治理 1：命名与变更分级；对照 1：课程 profile 与多课程 ID），只写「结论 + 指向项目内源文档的相对链接」、不复制正文、易变值不入正文；10 篇全部由既有 ADR-002/003/005/007/010/012/013/014/015/016、standards、接口档案与 config 课程卡**编译**（未新增事实），机器初编一律不标 verified human；新增 **ADR-017**（采用 OKF v0.2 作为工程记忆格式层，只叠加格式层、不平行重建，Bundle A 知识详解 frontmatter 后置试点）；零第三方依赖校验器 `scripts/okf_validate.py` 自全局 okf-wiki 技能 **vendor 进项目**（带来源头注记、勿本地分叉），系统 python3 直接跑。独立评审逐篇与源交叉核对（两张课程卡全部 ID、用户空间台账 8 门课、接口常量逐项命中），58 条相对链接 0 断链、校验 E0·W0
- Bundle A 试点：知识详解 OKF frontmatter + 飞书同步自动剥离（2026-09-08，ADR-017 格式层向知识成品延伸）：①`scripts/knowledge/resync_wiki_content.py` 新增 `strip_frontmatter()`，写入飞书前自动剥离顶部 `---...---`（仅认文件顶部连续块、正文 `---` 分隔线不误剥），4 项单元测试 + 真实文件回归全过；②试点 4 篇加标准档 frontmatter（type 词表 KnowledgePoint / Reference / ChapterIndex，字段含 sources 讲义+题目 paperId、generated、status、stale_after、自定义 chapter/exam_season/question_count），OKF 校验 E0（W 全为临时目录隔离假阳性），剥离后正文与备份逐字一致；③飞书端到端验证：从飞书实时恢复节点映射表后同步税法原则一篇，回读飞书原始内容确认顶部无裸露 YAML、正文四节完整；④AGENTS.md 新增 3.12 节固化 type 词表与规范，wiki-link-verification-sop 补剥离说明。外部知识详解 4 篇不入库（ADR-005），logs 映射表被 gitignore；全量 108 篇已于 2026-09-08 完成推广（批量脚本 `data/_workspace/batch_add_frontmatter.py` 遍历生成，OKF 校验 E0，剥离后正文与备份逐字一致，飞书 108/108 同步成功，回读抽查无裸露 YAML）；映射表 `logs/wiki_node_map.tsv` 从飞书实时遍历恢复（108 条）
- resync 健壮性优化（2026-09-08）：`scripts/knowledge/resync_wiki_content.py` ①重试次数 3→5，检测到 token 失效类错误（temporary token / Authorization / invalid_response）时等待 10 秒再重试（普通错误 2 秒）；②每 15 篇额外暂停 5 秒（可通过 `RESYNC_BATCH_SIZE` / `RESYNC_BATCH_PAUSE` 环境变量配置），避免连续调用 100+ 次 API 导致 lark-cli 临时 token 失效
- 名师课 H.265 重压双机并行 + 整科门控网盘 hevc 覆盖（2026-09-16，ADR-022）：六科约 2062 个 h264 1080P 改两台 mac 科目零重叠并行（本机压审计/战略/经济法/税法，目标机压会计/财管，数据 rsync 同构相对路径迁移、不走 git）。新增 `scripts/ep3_local_supervisor.sh`（本机 launchd LaunchAgent 常驻总管，RunAtLoad+KeepAlive 抗 Doubao 会话清理，替代曾夜间停摆 6h 的裸 nohup）、`scripts/netdisk_verify_summary.py`（终态核验分类汇总），`compress_ep3_videos.py` 增 `--jobs N`（x265 pools，按忙闲调档）/`--reverse`（倒序）；目标机两个 nohup caffeinate 看门（会计→财管压缩衔接、全 hevc 后限速上云）为 logs 过程件不入库。网盘侧确立"整科全 hevc 且压缩结束才传"门控、首次 hevc 上传清视频 done（保留 notes__ 讲义标记）+ rtype=3 同名覆盖早期 h264、两台各讲并发 1 + BAIDU_UPLOAD_RATE=1000k、终态以 verify_netdisk_final 集合+字节大小核验为准不信 done（网盘不能 ffprobe，靠 hevc 小/h264 大的大小差识别旧版）。文档：新增 ADR-022 与 2 篇 OKF concept（workflow-video-hevc-compression、workflow-netdisk-hevc-sync，index/log 登记）；video-processing 补双机并行/常驻保活/调档/踩坑小节，finalize-sop 步骤1 补 hevc 门控覆盖与双机限速，netdisk-final-verification 补大小判旧版。实测：审计 257、战略 129 全 hevc 上云终态核验 rc=0、视频 0 缺/0 大小不一致/0 旧大版；目标机 961 个视频与本机源 diff 一致、ffprobe 全可解析。AI 初编未标 verified
...
- 税法课知识系统 finalize 质量修复与三地同步（2026-09-06 至 09-07）：①用户笔记元数据彻底清理——先移除 524 处点赞数标注（commit d956c53），再移除 745 处「（学员 XXX）」标注 + 5 处正文残留来源标签（commit 3a8494e），`organize_user_notes.py` 根治为只输出纯文本内容；kill 正在同步带元数据版本的后台进程后重新同步，网盘 92/92 覆盖成功、飞书第一轮 90 OK + 18 限流 FAIL（集中 13/14 章）、等待 60 秒后重跑 18 篇全部补更成功，logs/wiki_done 回到 108/108；②飞书链接全量验证——三层 774 链接（总览页 28 + 章 README 92 + 知识点详解 654）逐篇回读提取 node_token 与 wiki_node_map.tsv 比对，0 错误 0 警告；③旧版归档节点清理——删除「【旧版归档】老方法15章结构」节点（含 16 个子节点，已被新 14 章结构完全覆盖），课程根下从 17 节点变为 16 节点；总览页同步删 00 行、链接文字改为 `[进入]`/`[速查]`、说明文字去掉「00 开班典礼保留早期独立结构」；④冲刺模考收尾检查——A1 冲刺视频已下载 49 文件夹 147 文件 13G（强化冲刺 4 套 + 模考 01 整卷+3题讲 + 02 整卷+4题讲 + 03 仅整卷，03 题目讲解高顿未出或漏下待确认）；A2 试卷/机考成绩分析——6 套考试（3 冲刺卷 + 3 机考）全部未满分，每套有 11–15 分 AI 不支持批改的主观题（aiUnsupported），最高 85.5 分/套，经用户决策放弃刷满分（知识详解已按 92 知识点全覆盖，人工批改分不可控且不额外花钱）；A3 答案质量复核——230 处「点拨」均为答案后附加解析（非混入答案本体），客观题/主观题格式正确，未发现空答案或半截答案
- 项目维护与结构评审：脚本冗余清理（2026-09-07，用户原则「以精简并删除历史冗余代码为荣，以堆彻重复实现为耻」）：全仓 79 个脚本评审，清理 19 个 0 代码引用的一次性/已替代脚本——7 个探查脚本（probe_exam_entry/quiz_dom/sprint_video/user_notes~4）、1 个测试（api_submit_test）、1 个 demo（sniff_demo）、4 个旧 Playwright UI 做题脚本（answer_multi/answer_option/submit_exam/playwright_connect）、3 个旧同步脚本（sync_wiki/update_wiki_content/update_wiki_v2）、2 个旧抓取脚本（capture_schedule_list 被 refresh_inventory 替代、capture_quiz_load 题源已采全）、1 个 UI 对照实验（answer_submit_capture）；脚本从 79 精简至 60。同步更新 scripts/README.md（删 16 条目、补 knowledge/migrate 两章 8 条目、计数对齐 60=60、使用原则改为纯接口主链路）、5 份现行操作文档（REQUIREMENTS/WORKFLOW/DOCUMENTATION_MAP/exam-workflow/browser-cdp-connect-guide）中对已删脚本的引用；ADR-010 与任务报告按「不改写历史」原则保留原文。decisions/README.md ADR 索引补登 013/014
- 项目维护：文档内容质量评审维度补全（2026-09-07）：抽样 9 篇文档（ADR×2、SOP/指南×3、规范×2、README×1、任务报告×1）逐篇评估精简度与专业度，结论为整体质量高、无"正确的废话"或需重写文档，仅 2 处需修复。①DOCUMENTATION_OPTIMIZATION.md 健康度检查清单"质量"维度原仅有"重叠内容"和"大文档分解"，补增 3 项——内容精简度（冗余段落/重复说同一件事）、内容专业度（术语一致性/AI套话/结论依据）、结构清晰度（空标题/空章节，如"计算/综合大题（0道大题、0个小问）"必须删）；②README.md 移除过时话术"刷新Playwright token"（纯接口后 Playwright 已非主链路）。AGENTS.md 3.7节同步加入核心价值观「以精简并删除历史冗余为荣，以堆彻重复实现为耻」，覆盖代码/文档/结构三个维度
- 工程记忆配套接入既有治理（2026-09-07）：①`scripts/pre-commit` 新增第 8 段——仅当暂存触及 `docs/project-management/memory/` 或 `scripts/okf_validate.py` 时跑 OKF 校验，E 硬错误 exit1 阻断、W/I 不阻断；第 5 段中文 Context Block 检查豁免 memory 目录（OKF 用 frontmatter type，受控词 Architecture/Workflow/Standard/Reference 与项目中文文档类型词是两套体系、不冲突）；②根 `AGENTS.md` 必读顺序与 3.10 新会话恢复顺序插入「读 memory/index」步骤、新增 3.11「工程记忆 OKF bundle 与项目自包含（强制）」；③`docs/DOCUMENTATION_MAP.md` 入口 / Concept 表 / 维护清单 / 优化记录 4 处登记；④`scripts/README.md` 检查类计数与 pre-commit 用途同步、新增 okf_validate.py 专节；⑤`decisions/README.md` ADR 索引补 017。实测：真实 hook 端到端 exit0、坏 frontmatter 负向夹具正确 exit1、修复后复跑 E0·W0
- 多角色协作通用方法论上提为全局技能（2026-09-14）：四职能角色通用定义、协作链路、领域专家角色卡模板抽至全局技能 `idea-to-tickets/references/role-collaboration.md`，确立两层模型——项目架构师/开发/产品/秘书为固定职能角色，珠宝/股票/CPA/房地产评估等领域专家按业务用角色卡扩展、技能不写死具体业务；本项目 `multi-role-collaboration.md` 瘦身为项目自包含配合（保留课程生产流水线分工与角色—文档映射，182→66 行），README 多角色段同步去重收敛、删易变的技能数列；其余 7 处对该文档的引用路径不变

### 修复
- 根治知识详解残留「### （二）计算 / 综合大题（0 道大题、0 个小问）」空标题（2026-09-06）：根因是 `render_point_qa.py` 无条件输出大题分类标题（`assemble_point.py` 本就有 `if groups:` 保护，52 篇为早期旧版渲染产物）。改为 `if groups:` 包裹、题量为 0 整段不渲染；`KNOWLEDGE_BASE_TEMPLATE.md` 同步补「空分类不渲染」硬约束（段说明+硬性约束各一条）。修复 Desktop/data 两份共 52 篇并校验 `diff -rq` 108 篇完全一致；52 篇覆盖重传网盘（0 失败）、重写对应飞书节点（复用原节点 overwrite、0 失败，批量回读 52 篇确认空标题为 0、题答解析完整）；清退被新结构 100% 覆盖的网盘旧「知识原料/knowledge-source」（116 套 papers，进回收站）与本地 data/knowledge-source（移废纸篓）。另实测确认：开放平台 OAuth token 沙箱限于 /apps，账号级回收站接口 recyclelist/cleanrecycle 稳定返回 31296（对照 listall 跨 /apps 报 31064 file is not authorized），无法用现有 token API 清空回收站，需账号主人在网盘端手动清空或等回收站 10 天自动到期
...
- 工程记忆评审顺带清理历史冗余与旧文件名口径（2026-09-07，呼应「以精简并删除历史冗余为荣」）：①`scripts/setup_data_symlink.sh` 删除重复段——原文件先 `set -euo pipefail`+`BASH_SOURCE` 定义一次目录，又用更弱的 `set -e`+`$0` 重复定义一遍；保留更健壮的前者、删重复（100→94 行），两处 `$1` 改 `${1:-}` 防 `set -u` 下无参数 unbound，`bash -n` 通过、`--check` 实测软链状态正常、行为不变；②现行文档旧文件名口径修正——`decisions/README` 退役表把 `setup-data-symlink.sh` 由「已不存在、改手动 ln」更正为「已 snake_case 更名为 `setup_data_symlink.sh`、仍自动建链」，`sniff_demo.js` 整合目标 `connectBrowser.js` 更正为现名 `connect_browser.js`；ADR-005 加 2026-09-07 演进注记（历史原文保留不改、相关文档清单指现名，对齐 ADR-010 既有范本）；`project-management/active/STAGE2_EXECUTION_PLAN.md` 现行计划中的 `connectBrowser.js` 同步改现名；③`scripts/pre-commit` 第 8 段把「🧠 检查 OKF」标题移入触发条件内，未暂存触及 memory/校验器时零输出，与「仅触及时运行」注释严格一致。历史快照（本 CHANGELOG、task-reports、ADR 历史验证段）按「只增不改」保留旧文件名，不改写历史
- 课程总览.md 定性为飞书同步派生源并清出课程库根与网盘（2026-09-07）：排查确认 `课程总览.md` 是 `build_course_overview.py --format markdown` 产出的飞书课程根总览页本地源（飞书正式版走 `--format xml` 现场生成、无脚本消费落盘 md、可随时再生成），不符合 finalize-sop「课程根只留 原始资源/知识详解」与网盘筛选「只传面向学习者成品（知识点4节篇+两课程全局篇）」，此前仅因 `upload_course.sh` 过滤漏配而上了网盘。处置：①`SKIP_PATTERNS` 增 `课程总览.md`（隔离实测命中、两全局篇仍正常上传）；②PROJECT_STRUCTURE_MAINTENANCE 3.1 上传分类表与 3.3 过滤清单各补一条规则闭环；③本地课程根散件移 `~/.Trash/gkb-overview-cleanup-*/`，课程根回到两目录；④网盘课程根副本先 list 取证（3998B 与本地一致）再 filemanager `opera=delete` 删除（进网盘回收站、可恢复），复查本地=网盘均只剩两目录。飞书总览页不受影响（需要时 xml 现场生成）
...
- 退役旧按讲取料路由整套（2026-09-06，用户拍板方案甲，闭环上一条遗留）：实测 `lecture-resource-map.json` 39 条全为已清退的「讲NN/」旧路径（现存 0 个、对盘必红）、新主流程零引用、配套 `verify_lecture_map.py`/`check_course_lib.py` 互为无调用叶子且硬编码旧路径，故 git rm 这 3 个文件；同步现行文档口径——DIRECTORY_STRUCTURE、PROJECT_STRUCTURE_MAINTENANCE（2.2.3 三问判据补 ADR-012 修订：课程级可 rebuild 路由不入库）、knowledge-base/README（重写为退役说明+三层桶指引）、scripts/README（删两段旧脚本说明）；新增 `scripts/migrate/README.md` 标注 M0–M6 为一次性迁移工具、新课程不直接套用。ADR-005/011/012、已自标废弃的 lecture-knowledge-build-sop、active 历史台账、任务报告按"不改写历史"保留；paper-manifest-sop/DOCUMENTATION_MAP 的演进注记保留
