...
### 4.3.1 主观题答案完整性校验门（多小问大题，2026-09-05 实锤后新增）

> **铁律：先保证答案与官方解析一致、完整覆盖题干要求的每一小问，再谈 AI 判分。有解析的题不能写错、不能写漏。**

**背景（2026-09-05 冲刺模考 45 题/1843142 实锤）**：顶层独立 type6 多小问大题的解析形态常为 `(1)(2)【点拨】…(3)…【点拨】…(4)`——**【点拨】穿插在小问之间，点拨之后的 (3)(4) 仍是正式答案**。旧 `canonAnswer` 用 `split('【点拨】')[0]` 只取第一个点拨之前，会把 (3)(4) 整段丢弃；同时 `replace(/^[（(]\d+[)）]/,'')` 剥掉开头 (1) 序号，导致首小问无编号、分点混乱。后果：平台"我的答案"残缺丢分、知识源质量受损，且易被误归为"平台 AI 误判"。

**机制（已内置 `gaodun_paper_core.js`）**：

| 步骤 | 函数 | 作用 |
|------|------|------|
| ① 数题干要求几问 | `countAskedSubs(title)` | 取题干「要求/根据上述资料」之后的连续序号 1..N，避开正文里的「业务（4）」「硫酸雾（100）」等噪声 |
| ② 多小问大题用完整解析 | `multiSubAnswer(analysis, N)` | 完整保留全部小问（不按点拨截断），去教辅标记但保留点拨内容（含采分语境、冗余不扣分），每个小问序号前换行分点 |
| ③ 完整性校验门 | `coveredSubs` + `answerGaps` | 提交前比对题干要求的 1..N 与答案实际覆盖，缺则显式记录 `answerGaps` 并用整段解析兜底，**禁止静默漏答** |
| ④ 单小问维持原逻辑 | `canonAnswer` | type5 子题或顶层单问（N=1）仍取点拨前精简版，不改动 |

**判读口径**：
- `answerGaps=[]` 是答案完整性的**硬验收线**，非空即说明有题漏小问、必须排查，不能宣布完成；
- 多小问大题 `cpaBotType===0`（未配 AI）：答案完整提交但平台不 AI 判分（须人工批改权益），分数不涨是平台限制、不是答案问题；
- `cpaBotType===3` 但 correct-ai 回 `11193401`：以接口实际返回为准（题目标记≠实际配了评分标准），归 unsupported，不反复重试；
- **区分"答案残缺"与"平台 AI 判分上限/波动"**：答案残缺是我方 bug（修 canon/校验门）；AI 判分上限是平台模型稳定误判（逐字/语境/分步三种表述仍 0 分，归 `aiCeiling`），二者不能混为一谈。
