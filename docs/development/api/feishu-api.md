...
> - **最终方案：换新进程批次法**（`scripts/knowledge/run_resync_batches.sh`，会计 169 篇实测收敛）：外层 shell 循环，每轮拉起**全新** `resync_wiki_content.py` 进程，靠 `RESYNC_MAX_NEW=N`（本轮新写满 N 篇即主动干净退出）控制单进程写量，轮间休眠再换新进程；连续**零新增**轮次休眠指数退避（`PAUSE*2^empty`、封顶 900s），有任意新增即归零；批次模式下进程内连续 2 败**快速退出**（不在单进程内长冷却空耗），把等待交给外层。推荐参数：每轮 8 篇、基础休 75s（写满 75s / 没写满 150s / 连续零新增 150→300→600→900s）。健康时约 50s 写满 8 篇，深限流自动退避、恢复后一轮补齐，done 断点保证任意中断/重入不丢、不重复耗配额。
> - **排查口诀**：再遇到 `invalid_response` / rc=5 / 非 JSON（e 开头），**先停脚本用全新 shell 手动单发同一篇对照**——新进程成功即代理层累计限流，直接走批次法；不要误判飞书账号、不要让用户重新登录、不要只靠一味加长静默。
> - **第二层根因：外层 shell 也会"凭证老化"，必须连外层一起周期换新（2026-09-12 第三遍补查）**：只在一个长期 `nohup` 外层里换 python 子进程仍不够——实测每个外层 shell 健康窗口约 **3 轮（24 篇、6~7 分钟）**，之后即使每轮都是新 python 进程也连续零新增；而 kill 外层、用**全新交互 shell** 重开后第一轮立即写满。故 `run_resync_batches.sh` 第 6 参数 `MAX_ROUNDS`（默认 3）让单个外层跑满 3 轮主动退出，由人/AI 用新 Bash（或 cron）重新拉起，done 断点无缝续跑。169 篇约 7 个外层、每个 6 分钟收敛。
