...
| CPA 层通用篇 | `通用做题思路解析.md` | 放在 `高顿/CPA/` 下，跨课一份 |
...
| CPA 层 | 通用做题思路解析 | `高顿/CPA/通用做题思路解析.md` |
...
1. 网盘**只镜像"原始资源 + 知识详解"两层**，结构与本地完全一致；前缀 `/apps/CPA课程归档/会计知识库/高顿/`；
