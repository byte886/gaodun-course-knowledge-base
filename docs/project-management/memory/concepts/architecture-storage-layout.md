...
type: Architecture
title: 四地存储分工与仓库版图
description: Git 仓库 / 项目内 data/高顿 实体目录 / 百度网盘 / 飞书知识库各放什么，data 实体目录与 gitignore 边界，以及"轻量派生元数据才入库"的判据。
tags: [storage, architecture, gitignore, 分工]
sources:
  - id: adr-005
  ...
  - id: adr-021
    resource: ../../decisions/ADR-021-数据实体入data去软链.md
    title: ADR-021 数据实体入 data/高顿、去桌面软链
  - id: directory-structure
    resource: ../../../DIRECTORY_STRUCTURE.md
    title: DIRECTORY_STRUCTURE 目录结构与文件命名规范
generated: { by: "doubao/okf-wiki", at: "2026-09-14T18:00:00+08:00" }
status: stable
---
...
| **本地大文件目录** `data/高顿`（项目内实体目录、整体 gitignore；课程在 `CPA/<课程>/`，无"课程库"中间层） | 原始视频、音频、压缩产物、课程原始资源 + 知识详解成品 | 工程脚本 |
...
## 关键规则
- **`data/高顿` 是项目内实体目录（ADR-021，2026-09-14 起去软链）**：数据直接放 `data/高顿/`、整体被 `.gitignore` 忽略，所有脚本走相对路径 `data/高顿/...`；不再有指向 `~/Desktop/高顿` 的软链，`find data/高顿` 直接遍历、**无需 `-L`**。换机/克隆后把数据放回 `data/高顿/`（从百度网盘镜像拉回）即可，不再用 setup_data_symlink（已退役）。
- **课程物理层无"课程库"中间层（ADR-020，2026-09-12）**：本地 `data/高顿/CPA/<课程>/{原始资源,知识详解}`、网盘 `/apps/CPA课程归档/会计知识库/高顿/CPA/<课程>`；跨课《通用做题思路解析.md》在 CPA 层（与各课程平级）。百度沙箱根名 `/apps/CPA课程归档` 与应用授权绑定，**禁止用 API 改名**（实测改名后写操作全报 31064、改回立即恢复），根内按领域分 `会计知识库/股票知识库/珠宝知识库`。飞书知识空间里的"课程库"是浏览层节点、与物理层解耦，不在此列。
...
- [ADR-021 数据实体入 data 去软链](../../decisions/ADR-021-数据实体入data去软链.md)（去桌面软链、data/高顿 实体化、setup_data_symlink 退役）
