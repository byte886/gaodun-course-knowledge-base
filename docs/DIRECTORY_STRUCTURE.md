...
| `_workspace/` 过程件（papers/manifest/留言原件/tmp/logs，finalize 后整体清退） | ❌ 忽略 | ✅（运行时） | ❌ 不传 | ❌ |
| 加密凭证 `.secrets/*.enc`、工具链 venv/node_modules | 见 .gitignore | ✅ | ❌ | ❌ |

**一句话**：Git 只放"怎么做"的工程资产；课程"原料 + 成品"在本地 data，原料与成品备份网盘，**成品知识系统最终落飞书**；过程件只在本地 `_workspace`、finalize 后整体清退。
...
│   ├── papers/                        # 题答 JSON（试卷题目/标准答案/官方解析/知识点标签），finalize 时从 _workspace 归位
│   └── user-notes/                    # 用户笔记/留言原件（按章分子目录），finalize 时从 _workspace 归位
│       └── NN_章节/
...
└── _workspace/                        # 第三层：运行过程件（英文命名，gitignore+不传网盘，finalize 后整体清退）
    ...
    ├── papers/                        # 题答 JSON（finalize 时归位到原始资源/papers/）
    ├── sniff/                         # CDP 抓包（验证完即删）
    ├── user-notes-raw/                # 用户留言/笔记原件（finalize 时归位到原始资源/user-notes/，100% 吸收进成品）
    ├── logs/
    └── tmp/
...
| 层 | 进什么 | 不进什么 | 生命周期 |
|----|--------|----------|----------|
| 原始资源 | 视频/转写、讲义/OCR、题答 JSON、用户笔记 | 过程件、临时件 | 永久，传网盘 |
| 知识详解 | 106 知识点篇 + 2 课程全局篇（共 108 篇） | 原始件、临时件 | 成品，可迭代，传网盘+同步飞书 |
| _workspace | manifest/papers/sniff/留言原件/logs/tmp | 成品、原始件 | 临时，finalize 后整体清退 |
