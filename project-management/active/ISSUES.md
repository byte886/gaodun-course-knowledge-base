...
| I-013 | **本地 JWT 7 天过期会让下载流水线静默空转**：消费者 `ep3_download_videos.js` 每次启动先调 `fetchSyllabus` 枚举阶段视频，用的是 `findJwt()` 读本地 auth 缓存；JWT 有效期 7 天，一过期所有新补位消费者枚举即返回 553649434「登录超时」FATAL 秒退，而 throttled 主循环 + 生产者（生产者走 CDP 实时 token，日志看似正常）都活着、反复补位反复秒退，表现为进程都在、实际 0 分片、15–20 分钟零产出，极具迷惑性。2026-09-11 12:1x 复现一次 | 2026-09-11 | **当前手动恢复**：跑 `node scripts/cdp/refresh_auth_token.js` 从已登录日常 Chrome 抓新 token（无需用户登录），调度器下一轮补位即自愈，实测恢复后 31MB/s。**待做的自动修复（L1，改脚本机制，方案待用户确认）**：消费者捕获 syllabus 553649434 时自动调一次 refresh 再重试，或让 throttled 主循环检测「连续 N 轮补位消费者秒退且日志含 553649434」时自动刷 token；同时定时任务/巡检判活不能只看进程数，要看分片字节增长（与 I-012 同一教训）。在自动续命落地前，每 7 天可能复发一次，巡检见分片零增长 + 消费者日志 FATAL 553649434 即按手动恢复处理 |
