...
- **视频/网盘线**：🔄 仍在另一窗口后台运行，但**不碰知识详解/**——会计已补齐、正通过有线传财管（只读写原始资源/videos）、战略3讲网盘补传；与本窗口知识库同步互不干扰

### 本窗口任务（飞书知识库同步）
- **当前阶段**：六科知识详解同步到飞书知识库
- **税法**：🔄 同步中（80/108篇，断点续传）
- **会计**：⏸️ A方案执行中——已删除32个旧节点（目录混乱：数字/汉字混用、排序错误），重新建树到14个节点时因bash脚本编码问题中断，已改用Python建树脚本（`scripts/knowledge/build_tree.py`），待继续
- **审计/财管/经济法/战略**：⏳ 待建树+同步（四科飞书根节点已创建，wiki_node_map.tsv不存在）
- **调度器**：Python版全自动调度器（`scripts/knowledge/auto_sync_all.py`）已开发，因会计建树中断暂停，待会计建树完成后重启
