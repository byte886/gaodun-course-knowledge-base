...
> 最后更新：2026-09-11（名师课补三科阶段配置：税法补全四阶段、战略/经济法补到三阶段，共新增 178 视频约 90G；config v2.1.0、throttled TASKS 15→22 行，新调度器已自动纳管新增阶段。修复单老师时代裸 meta 被完整性判据误读的机制问题 I-014：throttled / sync_ep3_ready 统一只认 `<老师>_meta.json`，可逆清理 304 个孤儿裸件 + 142 空目录（按 videoId 全局核对零真漏，成品零误伤）；**纠正调度器重启方法：禁用 `pkill -f`（新旧同命令行会双开），改按 PPID=1 的 throttled 根 BFS 收集后代 kill -9、确认残留 0 再启动**）
>
