...
5. **项目治理**：课程级过程件一律 `data/_workspace/<course>/` 不入库；稳定结论才提炼进 ADR/OKF/SOP。日常开发只 commit 不 push（除非用户要求）。
